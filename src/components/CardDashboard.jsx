...
export const CardDashboard = ({title, price,image,id,rate,count,producto}) => {
    ...
    return (
        <>
            ...
            <div className='flex flex-col min-w-56 gap-8 p-2 max-h-96 h-80 bg-white border text-letra border-gray-100 rounded-lg shadow-md group'>
                ...
                <Link href={`/dashboard/products/${id}`} className='h-1/2'><Image className='w-full h-full object-contain group-hover:scale-105 hover:cursor-pointer' src={image} alt='Imagen de producto' width={200} height={100}></Image></Link>
                <div>
                    <Link href={`/dashboard/products/${id}`} className='flex gap-2 hover:cursor-pointer'>
                ...
                <Link href={`/dashboard/products/${id}`} className='w-full flex gap-2 mt-2 relative hover:cursor-pointer'>
